...
public class UserData{
	...
	public UserData(string s){
		/*int l = s.Length, i, temp = 0;
		for (i = 0; i < l; i++) {
			if (s [i] == ' ')
				break;
		}
		username = s.Substring (temp, i - temp);
		...
		order = int.Parse (s.Substring (temp, i - temp));
		...
		score = int.Parse (s.Substring (temp, i - temp));

		temp = ++i;
		time = float.Parse (s.Substring (temp, l - temp));*/
		char[] c = { ' ' };
